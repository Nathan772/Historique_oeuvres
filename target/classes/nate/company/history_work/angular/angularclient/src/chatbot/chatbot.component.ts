    ...
    sendMessage() {
      if (this.textLastMessageContent.trim() !== "") {
        ...
        */
        ...
        //do actual response
        let responseFromLLMPromise = this.chatbotService.sendRequestToChatbot("I am the questioner, my name is : "+
          this.userService.userAccount.pseudo+", my question is : "+this.typeMessageUser);

        let responseFromLLM:string = "";
        responseFromLLMPromise.then(res =>
          {responseFromLLM = res

            })

        this.messages.push({text:responseFromLLM,authorId:this.aiName });
